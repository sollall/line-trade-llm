...
| `CHECK_MARGIN_PCT` | (任意・費用削減用)設定すると、直近足の安値〜高値の範囲をこの割合だけ広げた範囲にラインが入っていない足はLLMを呼ばずにスキップ | 未設定(全ラインを毎回判定) |
...
判定足を「1分」にしておくと、1分待つだけで判定を確認できます(Cronを手動実行するたびに、判定足が確定したすべてのラインでLLMが呼ばれます)。結果は画面の「状態」列と「判定履歴」、または `GET /lines/{id}/checks` で確認できます。
...
2. 価格との距離に関係なく、**全ラインを毎回**LLMに渡します。引いたラインそのもの(水平線は価格、トレンドラインは引いた2点と1足あたりの傾き、各足時点のラインの値)・直近足・**前回の状態と理由**を渡して状態を判定させ、`line_checks` に(送ったプロンプトごと)記録し、`lines` の現在の状態を更新します
...
呼び出し回数は「判定足1本につき、ライン1本あたり1回」です(例: 15分足のライン1本で1日96回、1時間足なら24回、日足なら1回)。費用を抑えたい場合は `CHECK_MARGIN_PCT` を設定すると、価格から離れたラインの判定をスキップできます。同じラインの同じ足の判定はKVにキャッシュし、Cronが同じ足をやり直しても再課金しません(仕様8)。
...
期間はHyperliquidが返せる直近5000本以内にしてください(15分足で約52日、1時間足で約208日)。期間内の**すべての足×ライン**を判定するので、`--llm claude` の呼び出し回数は「期間の足の本数×ライン数」になります(例: 15分足・ライン1本・1週間で672回)。まずは短い期間で試し、費用を抑えたい場合は `--margin-pct 0.002` で価格から離れた足をスキップできます。
